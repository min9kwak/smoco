...
class SplitBatchNorm3d(nn.BatchNorm3d):
    def __init__(self, num_features: int, num_splits: int, **kwargs):
        super(SplitBatchNorm3d, self).__init__(num_features, **kwargs)
    ...
    def convert_split_batchnorm(cls, module: nn.Module, num_splits: int = 8):
        """Helper function to convert `nn.BatchNorm3d` layers in the module to
        `SplitBatchNorm3d` layers. Works recursively.
        """
...
GhostBatchNorm3d = SplitBatchNorm3d


if __name__ == '__main__':

    # Note: batch_size % num_splits == 0
    import torch
    from models.backbone.resnet import build_resnet_backbone
    backbone = build_resnet_backbone(arch=18)

    backbone_sbn = SplitBatchNorm3d.convert_split_batchnorm(backbone, num_splits=8)

    arr = torch.randn(size=(16, 1, 96, 96, 96))
    out = backbone_sbn(arr)
