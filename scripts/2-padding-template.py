...
brain_files = [os.path.join(DATA_DIR, 'FS7', row.MRI, f'cat12/mri/wm{row.MRI}N.nii.gz')
               for _, row in data_info.iterrows()]

mri_save_dir = "D:/data/ADNI/template/FS7"
pet_save_dir = "D:/data/ADNI/template/PUP_FBP"
...
for _, row in tqdm.tqdm(data_info.iterrows(), total=len(data_info)):
    ...
    mri_file = os.path.join(DATA_DIR, 'FS7', row.MRI, f'cat12/mri/wm{row.MRI}N.nii.gz')
    if type(row.PET) == str:
        pet_file = os.path.join(DATA_DIR, 'PUP_FBP', row.PET, f'pet_proc/w_{row.PET}_SUVR.nii.gz')
    else:
        pet_file = None
