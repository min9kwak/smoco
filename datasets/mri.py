...
class MRI(Dataset):
    ...
    def load_image(self, path):
        image = self.slice_image(surface.load_surf_data(path))
        # image = image / 255
        return image
...
if __name__ == '__main__':
    ...
    for batch in train_loader:
        fig, axs = plt.subplots(1, 3, figsize=(15, 5))
        sns.heatmap(batch['x'][0, 0, 80, :, :], cmap='binary', ax=axs[0])
        sns.heatmap(batch['x'][0, 0, :, 80, :], cmap='binary', ax=axs[1])
        sns.heatmap(batch['x'][0, 0, :, :, 80], cmap='binary', ax=axs[2])
        plt.show()
        break
