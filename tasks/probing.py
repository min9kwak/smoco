...
class Probing(object):
    def __init__(self,
        ...
        self.demo2idx = {'gender': 0, 'age': 1, 'education': 2, 'apoe': 3, 'mmscore': 4}
        if self.target_name in ['gender', 'apoe']:
    ...
    def train(self, data_loader):
        ...
        with get_rich_pbar(transient=True, auto_refresh=False) as pg:
            ...
            for i, batch in enumerate(data_loader):
                with torch.cuda.amp.autocast(self.mixed_precision):
                    ...
                    if self.target_name in ['mc', 'volume']:
                        y = batch[self.target_name].to(self.local_rank)
                    elif self.target_name in self.demo2idx.keys():
                        y = batch['demo'][:, self.demo2idx[self.target_name]].to(self.local_rank)
                    else:
                        raise KeyError
        ...
        if self.task in 'classification':
            performance = classification_result(y_true=y_true.cpu().numpy(),
                                                y_pred=y_pred.softmax(1).detach().cpu().numpy(),
                                                adjusted=False)
        else:
            performance = regression_result(y_true=y_true.cpu().numpy(),
